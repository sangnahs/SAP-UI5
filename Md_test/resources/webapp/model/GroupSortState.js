sap.ui.define([
	...
	return BaseObject.extend("paint.model.GroupSortState", {
		 ...
		 * @alias paint.model.GroupSortState
		 */
		...
		/**
		 * Sorts by surface_type, or by surface_id
		 *
		...
		/**
		 * Groups by surface_id, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "surface_id") {
				...
				this._oViewModel.setProperty("/sortBy", "surface_id");

				aSorters.push(
					new Sorter("surface_id", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "surface_type");
			}

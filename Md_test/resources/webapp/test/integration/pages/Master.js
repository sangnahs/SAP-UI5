				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("surface_id") === oObject.surface_id;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].surface_id;
										} else {
											sCurrentId = aItemsNotInTheList[0].surface_id;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Paint_View", {
											surface_id : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("surface_id"),
							title: oBindingContext.getProperty("surface_type")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "Paint_View (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'Paint_View (0)'");
							},
					...
					theListShouldBeSortedAscendingOnUnitNumber : function () {
						return this.theListShouldBeSortedAscendingOnField("surface_id");
					},

					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("surface_type");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all Paint_View to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "Paint_View (" + iExpectedLength + ")"}),
									success : function () {
